<?php
	$page_title="View Channel Reports";
	$active_page="report";
	...
	function total_comments($post_id, $type='channel')
	{
		global $mysqli;
	...
	$sql="SELECT tbl_channels.*,tbl_category.`category_name` FROM tbl_channels
          ...
          WHERE tbl_channels.`id`='$id' ORDER BY tbl_channels.`id` DESC";

	$res=mysqli_query($mysqli,$sql);
	$row=mysqli_fetch_assoc($res);

 	$sql1="SELECT tbl_reports.*, tbl_users.`name` FROM tbl_reports, tbl_users 
 			WHERE tbl_reports.`user_id`=tbl_users.`id` AND tbl_reports.`post_id`='$id' AND tbl_reports.`type`='channel' ORDER BY tbl_reports.`id` DESC";
	...
	<div class="messaging">
		...
		<div class="heading">
			<div class="title" style="font-size: 16px">
				...
				<strong>Title:</strong>&nbsp;&nbsp;<?=$row['channel_title']?>
			</div>
		</div>
